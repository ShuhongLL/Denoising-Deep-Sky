...
class UNet(nn.Module):
    """
    Single-channel UNet (default in_nc=1, out_nc=1, nf=32).
    - Keeps the original structure, but fixes pooling layer wiring.
    - check_img_size pads to multiples of 16; exact-size tensors are not padded.
    - Returns output cropped back to the original (H, W).
    """
    def __init__(self, in_nc: int = 1, out_nc: int = 1, nf: int = 32):
        super().__init__()
        ...
        # Up 1
        self.upv6 = nn.ConvTranspose2d(nf * 16, nf * 8, kernel_size=2, stride=2)
        ...
        # Up 2
        self.upv7 = nn.ConvTranspose2d(nf * 8, nf * 4, kernel_size=2, stride=2)
        ...
        # Up 3
        self.upv8 = nn.ConvTranspose2d(nf * 4, nf * 2, kernel_size=2, stride=2)
        ...
        # Up 4
        self.upv9 = nn.ConvTranspose2d(nf * 2, nf, kernel_size=2, stride=2)
    ...
    def _pad_to_multiple(x: torch.Tensor, multiple: int = 16) -> torch.Tensor:
        """
        Pad feature map so H and W are multiples of 'multiple'.
        If already multiples, no padding is applied.
        """
        _, _, H, W = x.shape
        ...
        if pad_h == 0 and pad_w == 0:
            return x
        return F.pad(x, (0, pad_w, 0, pad_h))  # pad (left, right, top, bottom)
